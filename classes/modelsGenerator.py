...
class ModelsGenerator:
    ...
    def generate_models(self, deleteOldModels=False):
        ...
        for learning_rate in customizer.learningRates:
            for test_size in customizer.testSizes:
                for opt in customizer.optimizers:
                    for act in customizer.activators:
                        for neurnum in customizer.neurons:
                            for layerCount in customizer.hiddenLayers:
                                try:
                                    ...
                                    optim = customizer.get_optimizer(opt, learning_rate)
                                    model.compile(loss=loss, optimizer=optim, metrics=metrics)
                                    history = model.fit(x = X_train, y = Y_train, epochs = customizer.epochs, batch_size=32, validation_data = (X_val, Y_val))
    ...
    def create_model_by_params(self, parameters):
        trainingData = np.array(db.get_all_table_data("training_data"))
        X = trainingData[:, 1:-1]
        Y = trainingData[:, -1]
        X = tf.keras.utils.normalize(X, axis=1)
        X_train, X_val, Y_train, Y_val = train_test_split(X, Y, test_size=parameters.test_size)
        ln = 1
        neuronsNumber = parameters.neurnum
        model = tf.keras.Sequential()
        while ln <= parameters.layer_count:
            if (ln == 1):
                model.add(tf.keras.layers.Dense(parameters.neurnum, activation=parameters.activator))
                layerStr = f"{parameters.neurnum}"
            else:
                neuronsNumber = round(neuronsNumber / 2)
                model.add(tf.keras.layers.Dense(neuronsNumber, activation=parameters.activator))
                layerStr += f"/{neuronsNumber}"
            ln += 1
        print(layerStr)
        model.add(tf.keras.layers.Dense(1, activation='sigmoid'))
        optimizer = customizer.get_optimizer(parameters.optimizer, parameters.learning_rate)
        model.compile(loss=tf.keras.losses.BinaryCrossentropy(), optimizer=optimizer, metrics=[
            tf.keras.metrics.BinaryAccuracy(),
            tf.keras.metrics.Precision(),
            tf.keras.metrics.Recall()
        ])
